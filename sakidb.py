...
from radio_data import song_data, RadioItem
...
class Radio(dbase):
    __tablename__ = 'radio'
    artist = Column(types.String(length=30), primary_key=True)
    album = Column(types.String(length=128), unique=True)
    song = Column(types.String(length=20), nullable=True)
    timestamp = Column(types.DateTime, primary_key=True, nullable=False, default=datetime.datetime.now())
...
if __name__ == '__main__':
    ...
    parser.add_option("-a", "--add-radio", dest="add_radio", action="store_true", default=None, help="add radio")
...
#    add(session, period=60, agg_table=DataMinute)
    ...
    if options.add_radio:
        for key, item in song_data.items():
            ac = Radio(artist=key, album=item.album, song=item.song)
            session.add(ac)
        session.commit()